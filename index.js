...
class BitStreamClient {
    constructor(client, size) {
        this.client = client;
        this.size = size;
        this.dv = new DataView(new ArrayBuffer(this.size));
        this.offset = 0;
    }
    add(name, data) {
        const schemaID = this.client.schemaClientSendNameToId[name];
        if (schemaID == undefined) {
            throw new Error(`You id not create a schema for this message! ${name}`);
        }
        const { dataSchema } = this.client.schemasClientSend[schemaID];
        BitPacketEncoder[BitPacketTypes.Uint8].bind(this.dv)(this.offset, schemaID);
        this.offset += BitPacketBytes[BitPacketTypes.Uint8];

        for (const prop in dataSchema) {
            const binaryType = dataSchema[prop];
            const dataValue = data[prop];

            BitPacketEncoder[binaryType].bind(this.dv)(this.offset, dataValue);
            const bytes = BitPacketBytes[binaryType];
            // message is a string
            if (bytes === -1) {
                // length digit + whole string length
                this.offset += 1 + (dataValue.length & 0xff);
                continue;
            }
            this.offset += bytes;
        }
...
class Client {
    ...
    useStream(size = 0xffff) {
        if (this.hasStream) {
            throw new Error('You have already created a stream for this connection');
        }
        this.hasStream = true;
        this.stream = new BitStreamClient(this, size);
    }
    onSocketOpen() {
    ...
    parseBinaryPacket(binary) {
        ...
        if (offset < binary.byteLength) {
            this.parseBinaryPacket(binary.slice(offset));
        }
    }
...
class BitStreamServer {
    ...
    add(name, data) {
        const schemaID = this.connection.server.schemaServerSendNameToId[name];
        if (schemaID == undefined) {
            throw new Error(`You id not create a schema for this message! ${name}`);
        }
        const { dataSchema } = this.connection.server.schemasServerSend[schemaID];
        BitPacketEncoder[BitPacketTypes.Uint8].bind(this.dv)(this.offset, schemaID);
        this.offset += BitPacketBytes[BitPacketTypes.Uint8];
...
class WSConnection {
    ...
    useStream(size = 0xffff) {
        ...
        this.hasStream = true;
        this.stream = new BitStreamServer(this, size);
    }
